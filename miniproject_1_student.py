...
# Context: In this part, you are going to play around with some commonly used pretrained text embeddings for text search. For example, GloVe is an unsupervised learning algorithm for obtaining vector 
# representations for words. Pretrained on 2 billion tweets with vocabulary size of 1.2 million. Download from [Stanford NLP](http://nlp.stanford.edu/data/glove.twitter.27B.zip). 
...
def download_glove_embeddings_gdrive(model_type):
    ...
    gdown.download(
        url=f"https://drive.google.com/uc?id={word_index_id}",
        output=word_index_temp,
        quiet=False,
        fuzzy=True,
        resume=True,
    )
    ...
    gdown.download(
        ...
        output=embeddings_temp,
        quiet=False,
        fuzzy=True,
        resume=True,
    )

    # --- ADDED: sanity checks to catch HTML/partial downloads ---
    # If Drive returns an HTML error page, the file will be small and start with "<"
    if os.path.getsize(word_index_temp) < 100000:  # 100 KB
        with open(word_index_temp, "rb") as f:
            head = f.read(20)
        raise RuntimeError(f"word_index download looks invalid (size too small). First bytes: {head}")

    if os.path.getsize(embeddings_temp) < 1000000:  # 1 MB
        raise RuntimeError("embeddings download looks invalid (size too small).")
...
def load_glove_embeddings_gdrive(model_type):
    ...
    try:
        # Use latin1 (standard for python2 pickles)
        word_index_dict = pickle.load(open(word_index_temp, "rb"), encoding="latin1")
        embeddings = np.load(embeddings_temp)
        return word_index_dict, embeddings

    except Exception as e:
        # Corrupt / HTML / partial download -> delete + re-download + retry once
...
def cosine_similarity(x, y):
    ...
    # Safety: if dimensions mismatch, raise a clear error
    if x.shape[0] != y.shape[0]:
        raise ValueError(f"Embedding dimensions differ: {x.shape[0]} vs {y.shape[0]}")

    # Compute cosine similarity = (x·y) / (||x|| ||y||)
    x_norm = la.norm(x)
    y_norm = la.norm(y)

    # Avoid division by zero: if either vector is all zeros, similarity is 0
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    # Edge case: empty input sentence (or sentence with only spaces)
    if len(words) == 0:
        return embedding

    # keep a counter so we know how many words we averaged over.
    count = 0
    for w in words:
        # get_glove_embeddings returns:
        vec = get_glove_embeddings(w, word_index_dict, embeddings, model_type)

        # Add this word's vector into the running sum
        embedding += vec
        count += 1

    # average vector by dividing by number of words
    embedding /= count
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    """
    categories = [c for c in st.session_state.categories.split() if c]
    cosine_sim = {}
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        sentence_embedding = averaged_glove_embeddings_gdrive(
            st.session_state.sentence, word_index_dict, embeddings, model_type=model_type
        )

        # Compute cosine similarity between the input sentence and each category
        for index in range(len(categories)):
            category_embedding = averaged_glove_embeddings_gdrive(
                categories[index], word_index_dict, embeddings, model_type=model_type
            )
            cosine_sim[index] = cosine_similarity(sentence_embedding, category_embedding)
        ...
        # OpenAI input sentence embedding
        sentence_embedding = get_openai_embeddings(st.session_state.sentence, model_name=model_name)

        # category embeddings cache check
        if (cache_key not in st.session_state) or any(c not in st.session_state[cache_key] for c in categories):
            get_category_embeddings(embeddings_metadata)

        # Compute cosine similarity between the input sentence and every category
        for index in range(len(categories)):
            category_embedding = st.session_state[cache_key][categories[index]]
            cosine_sim[index] = cosine_similarity(sentence_embedding, category_embedding)
        ...
        # Transformer input sentence embedding
        sentence_embedding = get_sentence_transformer_embeddings(st.session_state.sentence, model_name=model_name)
    ...
    ##########################################
    sorted_cosine_scores_items = sorted(cosine_sim.items(), key=lambda kv: kv[1], reverse=True)
    return sorted_cosine_scores_items
...
if __name__ == "__main__":
    ...
    st.session_state["sentence"] = st.session_state["text_search"]
    ...
    if st.session_state.text_search:
        ...
        st.write("")
        st.write(
            "Demo developed by [Joe Zhang](https://www.linkedin.com/in/your_id/ - Optional)"
        )
